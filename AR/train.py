...
def save_checkpoint_atomic(payload: dict, target_path: str | Path) -> None:
...
def _schema_unique_ratio(schema: DatasetSchema, col_id: int) -> float:
    column = schema.columns[int(col_id)]
    ...
    if unique_ratio <= 0.0:
        ...
        unique_ratio = train_unique / max(float(schema.train_rows), 1.0)
    return float(np.clip(unique_ratio, 0.0, 1.0))


def _schema_identifier_signal(schema: DatasetSchema, col_id: int) -> float:
    column = schema.columns[int(col_id)]
    policy = dict(column.stats.get("policy") or {})
    role = str(policy.get("role", ""))
    if role in {"identifier", "quasi_identifier"}:
        return 1.0
    empirical_weight = float(policy.get("empirical_weight", 0.0) or 0.0)
    unique_signal = float(np.clip((_schema_unique_ratio(schema, col_id) - 0.02) / 0.18, 0.0, 1.0))
    if role in {"high_card_categorical", "entity_name", "geo_code", "code_categorical"}:
        return float(max(unique_signal, empirical_weight))
    return unique_signal


def _weighted_choice(indices: list[int], weights: list[float], generator: torch.Generator) -> int:
    if not indices:
        raise RuntimeError("Cannot sample from an empty index set.")
    weight_tensor = torch.tensor(weights, dtype=torch.float32)
    if float(weight_tensor.sum().item()) <= 0.0:
        return int(indices[int(torch.randint(len(indices), (1,), generator=generator).item())])
    selected = int(torch.multinomial(weight_tensor, 1, replacement=False, generator=generator).item())
...
def sample_afd_objective(
    ...
    if not searchable_rhs:
        raise RuntimeError("No searchable RHS columns available for AFD sampling.")
    rhs_weights = [
        1.0 + 0.5 * np.sqrt(_schema_unique_ratio(schema, col_id))
        for col_id in searchable_rhs
    ]
    rhs_col = _weighted_choice(searchable_rhs, rhs_weights, generator)

    lhs_pool = [col for col in schema.searchable_lhs_indices() if col != rhs_col]
    if not lhs_pool:
        raise RuntimeError("No searchable LHS columns available for AFD sampling.")
    ...
    use_weighted_lhs = bool(torch.rand((), generator=generator).item() < 0.70)
    if use_weighted_lhs:
        lhs_weights = [
            1.0
            + 2.0 * _schema_identifier_signal(schema, col_id)
            + 0.35 * np.sqrt(_schema_unique_ratio(schema, col_id))
            for col_id in lhs_pool
        ]
        lhs_order = torch.multinomial(
            torch.tensor(lhs_weights, dtype=torch.float32),
            lhs_size,
            replacement=False,
            generator=generator,
        ).tolist()
...
def train_model(config: PipelineConfig | None = None, force_preprocess: bool = False) -> dict[str, float]:
    ...
    for epoch in range(1, pipeline_config.training.max_epochs + 1):
        ...
        save_checkpoint_atomic(checkpoint_payload, pipeline_config.paths.last_checkpoint_path)
        ...
        if metrics["val_loss"] < best_val_loss:
            ...
            best_epoch = epoch
            no_improvement = 0
            save_checkpoint_atomic(checkpoint_payload, pipeline_config.paths.best_checkpoint_path)
        else:
            no_improvement += 1
