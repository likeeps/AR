...
from AR.column_policy import resolve_column_semantics
...
def _searchable_flags(runtime: DatasetRuntime, column_name: str, policy, search_space_mode: str) -> tuple[bool, bool]:
    semantics = resolve_column_semantics(
        name=column_name,
        policy=policy,
    ...
    return semantics.searchable_lhs, semantics.searchable_rhs
...
def profile_summary(config: "PipelineConfig") -> AutoProfileSummary:
    ...
    return _cached_profile(
        ...
        source_csv_path=config.paths.source_csv_path,
        source_fingerprint_token=json.dumps(source_fingerprint, sort_keys=True),
