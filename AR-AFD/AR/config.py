...
class PathConfig:
    ...
    def csv_path(self) -> Path:
        return self.repo_root_path / "traindata" / f"{self.dataset_name}.csv"

    @property
    def source_npy_path(self) -> Path:
        return self.repo_root_path / "traindata" / f"{self.dataset_name}.npy"

    @property
    def metadata_path(self) -> Path:
        return self.repo_root_path / "traindata" / f"{self.dataset_name}_meta.json"
...
def default_config(dataset_name: str | None = None) -> PipelineConfig:
    from AR.auto_profile import apply_auto_profile  # local import to avoid circular
    config = PipelineConfig(paths=PathConfig(dataset_name=dataset_name or DEFAULT_DATASET_NAME))
